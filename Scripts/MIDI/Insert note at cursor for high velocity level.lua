	...
	undo = 'Insert note at velocity '..vel
		...
		local retval, sel, mute, startpos, endpos, chan, pitch, vel = reaper.MIDI_GetNote(take, i)
			for _, t in ipairs(sel_note_t) do
				if startpos == t[2] and pitch == t[3] then reaper.MIDI_SetNote(take, i, true, mute, startpos, endpos, chan, pitch, vel, false) -- noSortIn - false since only one note prams are set
				end
			end
		end
	reaper.MIDI_Sort(take)
