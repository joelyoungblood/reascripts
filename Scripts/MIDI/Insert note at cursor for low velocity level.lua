...
if #IGNORE_MARGINS:gsub(' ','') > 0 then
	reaper.PreventUIRefresh(1)
	local stored_edit_cur_pos = reaper.GetCursorPosition()
	...
	reaper.SetEditCurPos(stored_edit_cur_pos, 0, 0) -- restore edit cursor pos; moveview is 0, seekplay is 0
...
local old_notecnt = reaper.GetExtState(cmd_ID..scr_name, 'note count') -- note count before calling the note 'insert' action
	...
	undo = 'Insert note at velocity '..vel
		...
		local retval, sel, mute, startpos, endpos, chan, pitch, vel = reaper.MIDI_GetNote(take, i)
			for _, t in ipairs(sel_note_t) do
				if startpos == t[2] and pitch == t[3] then reaper.MIDI_SetNote(take, i, true, mute, startpos, endpos, chan, pitch, VELOCITY, false) -- noSortIn - false since only one note prams are set
				end
			end
		end
	reaper.MIDI_Sort(take)
...
reaper.Undo_EndBlock('Insert medium velocity note at cursor', -1)
-- reaper.PreventUIRefresh(-1)
